...
try:
    import numpy as np
except ImportError:  # pygame can still run without optional generated audio
    np = None
...
def draw(surface, grid, piece, next_piece, score, lines, same_color_lines, fonts, game_over=False):
    surface.fill(BLACK)
    ...
    if game_over:
        ...
        draw_text(surface, fonts["title"], "SPIEL VORBEI", WHITE, screen_w // 2, screen_h // 2 - 70, True)
        draw_text(surface, fonts["normal"], f"Endstand: {score} Punkte", YELLOW, screen_w // 2, screen_h // 2, True)
...
def main() -> int:
    ...
    while True:
        ...
        if not game_over and now - last_drop >= drop_ms:
        ...
        draw(screen, grid, piece, next_piece, score, lines, same_color_lines, fonts, game_over)
        clock.tick(FPS)
